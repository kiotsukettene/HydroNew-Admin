...
class AnalyticsExportController extends Controller
{
    ...
    /**
     * Export analytics as PDF using DomPDF
     */
    public function exportPdfDompdf(Request $request)
    {
        try {
            // Get filters
            $filters = $request->only(['date_from', 'date_to', 'frequency', 'device_id', 'tab']);
            ...
            $filename = 'analytics-report-dompdf-' . now()->format('Y-m-d-His') . '.pdf';
            $this->exportService->htmlToPdfDompdf($html, $filename);
            ...
            return response()->json([
                'error' => 'Failed to generate PDF with DomPDF: ' . $e->getMessage()
            ], 500);
        }
    }
}
